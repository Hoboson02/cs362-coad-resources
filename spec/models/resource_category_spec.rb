...
RSpec.describe ResourceCategory, type: :model do
  ...
  describe 'activate' do
    it 'updates the current category from other to active' do
  ...
  describe '#deactivate' do
    it 'updates the current category from other to deactived' do
  ...
  describe '::active' do
    ...
    it 'returns all of the active resource categories in the database' do
      active_resource_category_one = create(:resource_category)
      active_resource_category_two = create(:resource_category)
      result = ResourceCategory.active
      expect(result.length).to eq(2)
      expect(result).to include(active_resource_category_one)
      expect(result).to include(active_resource_category_two)
    end

    it 'does not return inactive resource categories in the database' do
      create(:inactive_resource_category)
      result = ResourceCategory.active
  ...
  describe '::inactive' do
    it 'returns the inactive resource category in the database if there is only one' do
    ...
    it 'returns all of the inactive resource categories in the database' do
      inactive_resource_category_one = create(:inactive_resource_category)
      inactive_resource_category_two = create(:inactive_resource_category)
      result = ResourceCategory.inactive
      expect(result.length).to eq(2)
      expect(result).to include(inactive_resource_category_one)
      expect(result).to include(inactive_resource_category_two)
    end

    it 'does not return active resource categories in the database' do
